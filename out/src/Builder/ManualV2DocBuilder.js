 ...
 */
class ManualV2DocBuilder extends _DocBuilder2.default {
  ...
  exec({ writeFile, copy, readFile }) {
...

    {
      ...
      ice.load('content', this._buildManual(manualIndex), _iceCap2.default.MODE_WRITE);
      ice.load('nav', this._buildManualNav(manuals), _iceCap2.default.MODE_WRITE);
      ice.text('title', this._builderOptions.manualTitle || 'Manual', _iceCap2.default.MODE_WRITE);
      ice.drop('baseUrl');
    ...
    for (const manual of manuals) {
      ...
      ice.load('nav', this._buildManualNav(manuals), _iceCap2.default.MODE_WRITE);
      ice.text('title', manual.title, _iceCap2.default.MODE_WRITE);
      writeFile(fileName, ice.html);
    }
  ...
  _buildManualNav(manuals) {
    const ice = new _iceCap2.default(this._readTemplate('manualToc.html'));
  ...
  _getManualOutputFileName(filePath) {
    const parsed = _path2.default.parse(filePath);
...
exports.default = ManualV2DocBuilder;